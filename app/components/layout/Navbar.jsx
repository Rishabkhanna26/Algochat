...
export default function Navbar({ onMenuClick }) {
  ...
  useEffect(() => {
    ...
    const updateNavbar = () => {
      ...
      setNavElevated(currentScrollY > 8);
  ...
  return (
    <nav
      className={`flex min-h-16 w-full items-center justify-between gap-3 transform-gpu border-b border-gray-200 bg-white/95 px-3 py-2 backdrop-blur-sm transition-transform duration-300 ease-out will-change-transform sm:px-4 lg:px-6 ${
        navVisible ? 'translate-y-0' : '-translate-y-full'
      } ${navElevated ? 'shadow-sm' : ''}`}
      data-testid="navbar"
    >
      {/* Left: Menu + Search */}
