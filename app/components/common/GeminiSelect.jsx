...
export default function GeminiSelect({
  ...
  return (
    <div
      ref={wrapperRef}
      className={`aa-select aa-select--${variant} relative w-full ${className}`.trim()}
    >
      {label ? (
        <label
      ...
      >
        ...
        <FontAwesomeIcon
          icon={faChevronDown}
          className={`aa-select__chevron relative z-10 shrink-0 text-sm transition-transform duration-300 ${
            isOpen ? 'rotate-180' : ''
      ...
      {isOpen ? (
        <div
          role="listbox"
          className={`aa-select__menu absolute left-0 right-0 z-20 mt-2 max-h-72 overflow-auto ${
            isVibrant
          ...
          {options.map((option) => {
            ...
            return (
              <button
                ...
                className={`aa-select__option flex w-full items-center gap-3 border-b text-left transition-all duration-200 last:border-b-0 ${
                  isVibrant
