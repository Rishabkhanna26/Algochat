...
export default function DashboardPage() {
	...
	const growthTrendData = Array.isArray(stats?.growth_trend) && stats.growth_trend.length > 0
		...
		: overviewData;
	const overviewChartMax = getYAxisUpperBound(overviewData, ['value']);
	const growthTrendChartMax = getYAxisUpperBound(growthTrendData, ['value']);
	const leadStatusData = Array.isArray(stats?.lead_status_breakdown)
		? stats.lead_status_breakdown.map((item) => ({
			name: item?.label || String(item?.status || '').replace(/_/g, ' '),
			value: toCount(item?.value),
			status: item?.status || '',
			fill:
				LEAD_STATUS_COLORS[item?.status] || LEAD_STATUS_COLORS.pending,
		}))
		: [];
					...
					<div className="bg-white p-4 sm:p-6 rounded-lg border border-gray-200 shadow-sm">
						...
						<ResponsiveContainer width="100%" height={300}>
							<BarChart data={overviewData}>
								...
								<XAxis dataKey="name" />
								<YAxis allowDecimals={false} domain={[0, overviewChartMax]} />
								<Tooltip />
					...
					<div className="bg-white p-4 sm:p-6 rounded-lg border border-gray-200 shadow-sm">
						...
						<ResponsiveContainer width="100%" height={300}>
							<LineChart data={growthTrendData}>
								...
								/>
								<YAxis allowDecimals={false} domain={[0, growthTrendChartMax]} />
								<Tooltip />
						...
						{totalLeadStatuses > 0 ? (
							<>
									...
									<PieChart>
										...
										>
											{leadStatusData.map((item) => (
												<Cell key={item.status || item.name} fill={item.fill} />
											))}
										</Pie>
										<Tooltip />
									</PieChart>
								...
								<div className="mt-4 space-y-3">
									{leadStatusData.map((item) => (
										<div key={item.status || item.name} className="flex items-center justify-between gap-3">
							...
							<div className="flex h-[300px] items-center justify-center text-center text-sm text-gray-500">
