...
export default function AppointmentsPage() {
  ...
  const renderStatusSegmented = (current, onChange, disabled = false, size = 'sm') => {
    const sizeClass = size === 'sm' ? 'min-h-[36px] px-3 py-1.5 text-xs' : 'min-h-[40px] px-4 py-2 text-sm';
    return (
      <div
        className={`grid w-full grid-cols-1 gap-2 rounded-[24px] border border-gray-200 bg-white p-1 min-[480px]:grid-cols-3 ${disabled ? 'opacity-60' : ''}`}
        role="radiogroup"
        ...
        {statusOptions.map((option) => {
          ...
          return (
            <button
              ...
              disabled={disabled}
              className={`${sizeClass} inline-flex w-full items-center justify-center rounded-full border font-semibold transition ${
                active
                  ? option.activeClass
