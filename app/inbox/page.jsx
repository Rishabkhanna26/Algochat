...
export default function InboxPage() {
  ...
  const loadThreadMessages = async (userId, { reset = false } = {}) => {
    ...
    } else if (threadViewportRef.current && selectedThread === userId) {
      pendingThreadScrollRestoreRef.current = {
        userId,
        previousScrollHeight: threadViewportRef.current.scrollHeight,
        previousScrollTop: threadViewportRef.current.scrollTop,
      };
    }
    setThreadMap((prev) => ({
      ...prev,
  ...
  useEffect(() => {
    if (!selectedThread || isThreadLoading) return;
    const viewport = threadViewportRef.current;
    if (!viewport) return;

    const pendingRestore = pendingThreadScrollRestoreRef.current;
    if (pendingRestore?.userId === selectedThread) {
      viewport.scrollTop =
  ...
  }, [selectedThread, activeMessages.length, isThreadLoading]);
  ...
  return (
    ...
    >
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white/90 p-3 shadow-2xl backdrop-blur-sm sm:rounded-3xl sm:p-4 lg:p-5 xl:p-6">
          ...
          <div className="mt-4 grid gap-4 sm:mt-5 sm:gap-5 lg:mt-6 lg:h-[calc(100vh-18rem)] lg:min-h-[40rem] lg:grid-cols-[minmax(300px,380px)_minmax(0,1fr)] xl:grid-cols-[420px_minmax(0,1fr)] lg:items-stretch">
            <div
              className={`${mobileThreadOpen ? 'hidden lg:flex' : 'flex'} min-h-0 flex-col space-y-3 overflow-hidden sm:space-y-4 lg:h-full`}
            >
              <Card
                unstyled
              ...
              <Card
                unstyled
                className="order-1 flex min-h-[18rem] flex-1 flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white p-0 shadow-sm lg:order-2 lg:min-h-0"
              >
            ...
            <Card
              unstyled
              className={`min-h-[24rem] overflow-visible rounded-2xl border border-gray-200 bg-white p-0 shadow-sm lg:flex lg:h-full lg:min-h-0 ${mobileThreadOpen ? 'block' : 'hidden lg:block'}`}
            >
              {!selectedThread ? (
                <div className="flex min-h-[24rem] flex-1 flex-col items-center justify-center px-4 py-12 text-center sm:min-h-[28rem] sm:px-6 lg:min-h-0">
                ...
                <div className="flex min-h-[24rem] flex-1 flex-col sm:min-h-[30rem] lg:min-h-0">
                  <div className="flex min-h-0 flex-1 flex-col">
                    <div className="sticky top-[4.5rem] z-20 flex flex-col gap-2 border-b border-gray-200 bg-white/95 px-3 py-3 backdrop-blur-sm sm:gap-3 sm:px-4 sm:py-4 lg:top-[5.25rem] lg:flex-row lg:items-center lg:justify-between lg:px-5 lg:py-5">
                      <div className="flex items-center gap-2 sm:gap-3">
                        <button
                          type="button"
                    ...
                    <div
                      ...
                      className="flex-1 min-h-0 space-y-2 overflow-y-auto px-3 py-3 sm:space-y-3 sm:px-4 sm:py-4 lg:px-5 lg:py-5"
                    >
                      {isThreadLoading ? (
