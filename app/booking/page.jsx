  ...
  const handleSave = async (event) => {
    ...
    if (form.payment_required) {
      if (!(Number.isFinite(parsePriceAmount(form.price_label)) && parsePriceAmount(form.price_label) > 0)) {
        setSaving(false);
        setError('Paid booking items must include a numeric price.');
        return;
      }
    }
    ...
    <div className="space-y-6" data-testid="booking-page">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.5fr_0.9fr]">
        ...
        <div className="space-y-4">
          <Card
            unstyled
            className="rounded-xl border border-[#f1dcc5] bg-gradient-to-br from-white via-[#fff9f2] to-[#fdebd7] p-5 text-aa-dark-blue shadow-sm"
          >
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-aa-orange">
              How To Use
            </p>
            <div className="mt-4 grid gap-3">
              {BOOKING_SETUP_STEPS.map((step) => (
                <div
                  key={step.number}
                  className="flex items-start gap-3 rounded-2xl border border-[#f1dcc5] bg-white/90 p-4 shadow-sm"
                >
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-r from-[#FE8802] to-[#FDA913] text-sm font-bold text-white shadow-sm">
                    {step.number}
                  </div>
                  <div>
                    <p className="font-semibold text-aa-dark-blue">{step.title}</p>
                    <p className="mt-1 text-sm text-aa-gray">{step.description}</p>
                  </div>
                </div>
              ))}
            </div>
            <div className="mt-4 space-y-4 text-sm text-aa-gray">
              <div className="rounded-2xl border border-[#f1dcc5] bg-white/90 p-4 shadow-sm">
                <p className="font-semibold text-aa-dark-blue">Hotel</p>
              ...
              <div className="rounded-2xl border border-[#f1dcc5] bg-white/90 p-4 shadow-sm">
                <p className="font-semibold text-aa-dark-blue">Restaurant</p>
              ...
              <div className="rounded-2xl border border-[#f1dcc5] bg-white/90 p-4 shadow-sm">
                <p className="font-semibold text-aa-dark-blue">Events</p>
          ...
          <Card className="p-5">
            ...
            <div className="mt-4 rounded-2xl border border-gray-200 bg-gray-50 p-4">
              ...
              <p className="mt-3 text-xs text-aa-gray">
                Payment: {form.payment_required ? `Pay at ${selectedCategoryLabel} / Pay Now / Partial Pay` : 'Free booking'}
              </p>
            </div>
          </Card>
        </div>
      ...
      <Modal
        ...
        size="lg"
      >
        <form className="booking-editor space-y-4 sm:space-y-5" onSubmit={handleSave}>
          <div className="booking-editor-shell rounded-[28px] border border-[#f1dcc5] bg-gradient-to-br from-[#fff9f2] via-white to-[#fdebd7] p-4 shadow-sm sm:p-5">
            ...
            <div className="booking-editor-mobile-steps mt-4 grid gap-2 rounded-2xl border border-white/80 bg-white/85 p-4 md:hidden">
              {BOOKING_SETUP_STEPS.map((step) => (
                <div key={`mobile-step-${step.number}`} className="flex items-start gap-3">
                  <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-aa-orange text-xs font-bold text-white">
                    {step.number}
                  </span>
                  <div>
                    <p className="text-sm font-semibold text-aa-dark-blue">{step.title}</p>
                    <p className="mt-1 text-xs text-aa-gray">{step.description}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="mt-5 hidden gap-3 md:grid md:grid-cols-3">
              <div className="booking-editor-tip-card rounded-2xl border border-white/80 bg-white/80 p-4 shadow-sm">
              ...
              {BOOKING_CATEGORY_PRESETS.map((preset) => {
                ...
                return (
                  <button
                    ...
                    className={`booking-editor-preset-card group relative overflow-hidden rounded-[26px] border p-3 text-left transition-all duration-300 sm:p-4 ${
                      isSelected
                        ? 'booking-editor-preset-card--active border-[#FDA913] bg-gradient-to-r from-[#FE8802] to-[#FDA913] text-white shadow-2xl shadow-[#FE8802]/25'
                  ...
                  >
                    ...
                    <div className="relative z-10 flex items-start justify-between gap-3">
                      <div
                        className={`flex h-10 w-10 items-center justify-center rounded-2xl shadow-sm sm:h-11 sm:w-11 ${
                    ...
                    <p className={`relative z-10 mt-3 text-base font-bold sm:mt-4 sm:text-lg ${isSelected ? 'text-white' : 'text-aa-dark-blue'}`}>
                      {preset.label}
                    </p>
                    <p className={`relative z-10 mt-1 text-xs sm:text-sm ${isSelected ? 'text-white/85' : 'text-aa-gray'}`}>
                      {preset.description}
                    </p>
                    <p
                      className={`relative z-10 mt-3 hidden text-xs font-semibold uppercase tracking-[0.18em] sm:block ${
                        isSelected ? 'text-white/70' : 'text-aa-gray'
                      }`}
                    >
                      Good for
                    </p>
                    <div className="relative z-10 mt-4 hidden flex-wrap gap-2 sm:flex">
                    ...
                    <p className={`relative z-10 mt-2 text-xs font-semibold sm:hidden ${isSelected ? 'text-white/80' : 'text-aa-gray'}`}>
            ...
            <div className="mt-5 grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
              <div className="booking-editor-custom-panel rounded-[24px] border-2 border-[#FDA913] bg-white p-4 shadow-lg shadow-[#FDA913]/10">
                ...
                <div className="booking-editor-custom-list mt-4 rounded-2xl bg-gradient-to-r from-[#fff7ee] to-[#fff0da] p-4">
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-aa-gray">
              ...
              <details className="booking-editor-help-mobile rounded-[24px] border border-[#dbe4ff] bg-[#f5f8ff] p-4 text-aa-dark-blue lg:hidden">
                <summary className="cursor-pointer list-none text-sm font-semibold">
                  Quick help for this booking form
                </summary>
                <div className="mt-3 space-y-3 text-sm text-aa-gray">
                  <p>
                    {selectedPreset
                      ? `Use ${selectedPreset.label} when customers ask for ${selectedPreset.examples.slice(0, 3).join(', ')}.`
                      : 'Use a clear booking name and a short customer message so WhatsApp can match it correctly.'}
                  </p>
                  <div className="booking-editor-help-card rounded-2xl border border-[#e7eefc] bg-white p-3">
                    <p className="font-semibold text-aa-dark-blue">Booking name</p>
                    <p className="mt-1 text-xs text-aa-gray">Write the exact item customers will book.</p>
                  </div>
                  <div className="booking-editor-help-card rounded-2xl border border-[#e7eefc] bg-white p-3">
                    <p className="font-semibold text-aa-dark-blue">Search words</p>
                    <p className="mt-1 text-xs text-aa-gray">Only add extra keywords if people may search using different terms.</p>
                  </div>
                </div>
              </details>

              <div className="booking-editor-help-desktop hidden rounded-[24px] bg-aa-dark-blue p-4 text-white shadow-sm lg:block">
          ...
          <div className="booking-editor-step-card rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="w-full">
              ...
              <div className="booking-editor-current-category group relative overflow-hidden rounded-xl bg-gradient-to-r from-[#FE8802] to-[#FDA913] px-4 py-3 text-white shadow-lg sm:rounded-2xl sm:px-5 sm:py-4">
                <span className="absolute inset-0 -translate-x-full bg-white/10 transition-transform duration-700 group-hover:translate-x-full" />
          ...
          <p className="-mt-1 text-xs text-aa-gray sm:hidden">
            Use the cards above if you want to change the category.
          </p>

          <p className="-mt-1 hidden text-xs text-aa-gray sm:block">
          ...
          <label className="flex items-center justify-between gap-4 rounded-2xl border border-[#f1dcc5] bg-[#fff9f2] px-4 py-4">
            <div>
              <p className="text-sm font-semibold text-aa-text-dark">Require payment before booking</p>
              <p className="mt-1 text-xs text-aa-gray">
                If enabled, WhatsApp shows Pay at {selectedCategoryLabel}, Pay Now, and Pay Partial.
              </p>
            </div>
            <span className="relative inline-flex items-center">
              <input
                ...
                onChange={(event) =>
                  setForm((prev) => ({ ...prev, payment_required: event.target.checked }))
                }
          ...
          <details className="booking-editor-advanced rounded-2xl border border-gray-200 bg-white p-4 md:hidden">
            <summary className="cursor-pointer list-none text-sm font-semibold text-aa-dark-blue">
              Advanced options
            </summary>
          ...
          <label className="booking-editor-visibility flex items-center justify-between gap-4 rounded-2xl border-2 border-[#FDA913] bg-gradient-to-r from-white to-[#fff4e6] px-4 py-4 shadow-lg shadow-[#FDA913]/10 transition-all duration-300">
            <div>
