...
const renderLeadStatusLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent, name }) => {
...
export default function ReportsPage() {
  ...
  const leadChartData = reports?.leadStats?.map(stat => ({
    name:
      ...
      String(stat?.status || '')
        ...
        .join(' '),
    value: Number(stat?.count || stat?.value || 0),
    status: String(stat?.status || '').toLowerCase(),
    fill: LEAD_STATUS_COLORS[String(stat?.status || '').toLowerCase()] || '#0A1F44',
  ...
  const messageTimelineMax = getYAxisUpperBound(messageChartData, ['messages']);
  const totalLeadStatusCount = leadChartData.reduce((sum, item) => sum + item.value, 0);
  const leadStatusLegend = leadChartData.map((item) => ({
    ...item,
    percent: totalLeadStatusCount > 0 ? ((item.value / totalLeadStatusCount) * 100).toFixed(0) : '0',
  }));
  const rangeLabel = {
    '7days': 'Last 7 days',
    '30days': 'Last 30 days',
  ...
  return (
    <div className="aa-page-shell aa-stagger-children" data-testid="reports-page">
      {/* Header */}
      <div className="aa-page-header">
        <div className="aa-page-header__body">
          <h1 className="aa-page-title">Reports & Analytics</h1>
          <p className="aa-page-subtitle">Track your business performance and insights with cleaner cards and better chart spacing on smaller screens.</p>
        </div>
        <div className="aa-page-actions w-full sm:w-auto">
          <GeminiSelect
            label="Date Range"
            value={dateRange}
          ...
          <div className="h-[320px] sm:h-[360px] w-full">
            {totalLeadStatusCount > 0 ? (
              <div className="flex h-full flex-col gap-4">
                <div className="h-[240px] flex-1 sm:h-[280px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        data={leadChartData}
                      ...
                      >
                        {leadChartData.map((entry, index) => (
                          <Cell key={`cell-${entry.status || index}`} fill={entry.fill} />
                        ))}
                      </Pie>
                      <Tooltip
                        formatter={(value, _name, entry) => [
                          value,
                          entry?.payload?.name || 'Lead Status',
                        ]}
                      />
                    </PieChart>
                  </ResponsiveContainer>
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  {leadStatusLegend.map((item) => (
                    <div key={item.status || item.name} className="flex items-center justify-between gap-3 rounded-xl bg-gray-50 px-3 py-2">
                      ...
                      <span className="text-sm font-bold text-aa-dark-blue">{item.percent}%</span>
                    </div>
                  ))}
                </div>
              </div>
            ) : (
              <div className="flex h-full items-center justify-center text-center text-sm text-aa-gray">
